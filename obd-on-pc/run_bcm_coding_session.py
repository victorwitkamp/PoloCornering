"""
run_bcm_coding_session.py - thin BCM coding session runner.
...
Usage:
    python run_bcm_coding_session.py
    python run_bcm_coding_session.py --write
    python run_bcm_coding_session.py --operation restore-expert-backup-no-turn-signal-fogs --write
    python run_bcm_coding_session.py --operation clear-byte12-bit0 --write
    python run_bcm_coding_session.py --set-bit byte21-bit2 --clear-bit byte21-bit7 --write
    python run_bcm_coding_session.py --port COM10 --baud 38400 --write
"""
...
def _operation_label(operation: str, set_bits: Sequence[str] = (), clear_bits: Sequence[str] = ()) -> str:
    if set_bits or clear_bits:
...
def _run_read_phase(ser: serial.Serial) -> tuple[str, str] | None:
    ...
    b12_b0 = bit_state(current_coding, 12, 0)  # Nordic/Standlicht parking behavior hypothesis
...
def _run_write_phase(
    ...
    *,
    ...
    """
    print(f"\n=== Phase 2: {_operation_label(operation, set_bits, clear_bits)} ===")

    target_coding = _target_coding(current_coding, operation, set_bits, clear_bits)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (set_bits or clear_bits) and operation != OPERATION_RESTORE_EXPERT_BACKUP:
        parser.error("--set-bit/--clear-bit cannot be combined with --operation; choose one target style")
    try:
    ...
    run_id_base = _bit_changes_run_id(set_bits, clear_bits) if (set_bits or clear_bits) else operation.replace("-", "_")
    run_id = f"{run_id_base}_{ts}"
    args.log_dir.mkdir(parents=True, exist_ok=True)
    log_path = args.log_dir / f"{run_id}_session.txt"

    print(f"BCM coding session  run_id={run_id}")
    print(f"operation={operation}  target={_operation_label(operation, set_bits, clear_bits)}")
    if set_bits:
    ...
    with serial.Serial(args.port, args.baud, timeout=0.1) as ser:
        ...
        if not args.write:
            target_coding = _target_coding(current_coding, operation, set_bits, clear_bits)
            print(f"\nRead-only pass complete. Target for {_operation_label(operation, set_bits, clear_bits)}: {target_coding}")
            if normalize_coding(current_coding) == normalize_coding(target_coding):
        ...
        ok = _run_write_phase(
            ...
            log_dir=args.log_dir, operation=operation, set_bits=set_bits, clear_bits=clear_bits, run_id=run_id,
        )
        if not ok:
            _tp20_close(ser)
            return 1
