...
- The current long coding was captured and compared against the
  supplied/reference coding. That pair differs by two bits, both now
  live-tested and behavior-ruled-out for the current symptom:
...
### Offline PQ25 setting catalog

- Copilot's live trace/Gadget route remains useful, but it currently cannot be
  exercised because the phone is not available.
- The older static cornering-label map was replaced by the broader exact-car
  setting catalog. The retained outputs are:
...
- The catalog is generated by combining the retained Carista supported-settings
  source snapshot with the PQ25/XPL long-coding candidates:

```text
carista_apk_analysis/carista_supported_settings_map.json
carista_apk_analysis/build_pq25_carista_catalog.py
```
...
- Static analysis found the native strings, but did not find simple direct code
  ...
  practical map from app label to the supplied/reference coding pair, not proof
  of the exact Carista setting object or proof that those bits fix the visible
  symptom.
...
- Active exact-car catalog script:
...
expands them into the final raw coding value. The candidate final 6-byte value
chunks are known from the current/supplied-reference coding pair, but live tests
showed they are not the visible fix by themselves. They are also not the same
thing as the UI requested-value byte array.
