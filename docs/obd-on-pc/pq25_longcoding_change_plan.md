...
```powershell
python .\decode_pq25_longcoding.py --coding-file .\logs\pq25_next_baseline_220600_direct_read_summary.json --output ..\docs\obd-on-pc\logs\pq25_next_baseline_220600_settings_report.txt
```

## Candidate Change Profiles

The safest active path is report-only: decode the fresh `220600` or `5A9A`
payload, then derive only the two 6-byte Carista value chunks through
...
Do not use the old aggressive reference-matching helper. It copied focus bytes
from the known-good coding:
...
The obsolete full-long-coding writer was removed. The only remaining guarded
writer is the structured Carista tuple sender:

```text
obd-on-pc/write_carista_3b9a_tuple.py
```

It defaults to dry-run and accepts only the recovered compact tuple shape:

```text
3B9A + 6-byte value + 4-byte rawAddress4 + coding-type/tail suffix
```
...
For the current two-bit cornering delta, use `compose_carista_3b9a_tuple.py` to
derive `value6` only. Do not execute `write_carista_3b9a_tuple.py` until real
`rawAddress4`, coding type, and tail bytes have been recovered.
