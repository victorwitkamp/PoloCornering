...
def car_setting_cornering_lights_via_fogs_left() -> VagUdsAdaptationSetting:
    return VagUdsAdaptationSetting(
        ...
        native_source="Older ARM instruction-window recovery; official Play 9.8.3 x86 same-key helpers resolve to VagUdsAdaptationSetting",
        ...
        proof_notes=(
            ...
            "The older ARM per-side region loads part-number patterns 5Q0937084* and 6C093708*, but no 6R0937087K / 6R0-937-08x pattern was recovered for this path.",
            ...
            "Direct Play 9.8.3 x86 ELF audit shows the same left key has two type-7 helpers: 0x012da25a -> 0x0136c910 -> PLT 0x0197e2f0 VagUdsAdaptationSetting and 0x012da39a -> 0x01356f90 -> PLT 0x0197d6f0 VagUdsAdaptationSetting.",
...
def car_setting_cornering_lights_via_fogs_right() -> VagUdsAdaptationSetting:
    return VagUdsAdaptationSetting(
        ...
        native_constructor="VagUdsAdaptationSetting(VagUdsEcu, StringWhitelist, rawAddress, valueOffset, mask, key, MultipleChoiceInterpretation)",
        native_source="Older ARM instruction-window recovery plus official Play 9.8.3 x86 same-key VagUdsAdaptationSetting helpers; branch unresolved",
        ...
        proof_notes=(
            ...
            "Direct Play 9.8.3 x86 ELF audit shows the same right key has two type-7 helpers: 0x012da5a9 -> 0x01368690 -> PLT 0x0197e0a0 VagUdsAdaptationSetting and 0x012da6e9 -> 0x0136caf0 -> PLT 0x0197e300 VagUdsAdaptationSetting.",
            "The first x86 branch derives its whitelist from GOT [EBX-0x2AA4] CENTRAL_ELEC_MQB_ALL plus [EBX-0x2C9C] CENTRAL_ELEC_MK8; the second x86 branch uses GOT [EBX-0x2C94] UDS_CAN_GATEWAY_MEB plus [EBX-0x5DF4] CAN_GATEWAY.",
