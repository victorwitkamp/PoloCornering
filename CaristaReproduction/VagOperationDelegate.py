    ...
    return (
        ...
        VagOperationDelegateRawValueDispatchCase(
            ...
            evidence=(
                ...
                "Play 9.8.3 x86 0x016b9dc0 names VagOperationDelegate::readVagUdsSubmoduleValue; it resolves a raw address through vtable slot +0x154, constructs a command object from that value and the uint16 submodule id, then runs it.",
                "Play 9.8.3 x86 GetSubmoduleIdsOverUdsCommand::processPayload at 0x00c4e9b0 accepts exactly 12 bytes, splits two 6-byte MSB-first bitmaps, and ORs second-bank IDs with 0x80 via addSubmodulesIdsToResult at 0x00c4ee20.",
                "Play 9.8.3 x86 GetVagUdsSubmoduleIdsCommand::processPayload at 0x00c51ef0 accepts even-length 16-bit entries and keeps nonzero low-byte IDs only when the high byte is zero.",
            ),
...
def VagOperationDelegate_getVagSettingAvailabilityForEcuRoutes() -> tuple[VagOperationDelegateAvailabilityRoute, ...]:
    return (
        VagOperationDelegateAvailabilityRoute(
            ...
            native_route="setting vtable slot +0x3c -> VagSetting::isSubmodule before the AvailBy switch",
            predicate_input="VagSetting type word at offset +0x0c, then VagEcuInfo submodule vector at +0x10",
            availability_result="setting types 4, 5, and 9 route through VagOperationDelegate::getVagSettingAvailabilityForSubmodule; it returns the setting StringWhitelist::itemMatches result for any submodule VagEcuInfo full_scan_part_number value at +0x08",
            evidence=(
                "Play 9.8.3 x86 Ghidra export decompiles _ZNK10VagSetting11isSubmoduleEv at 0x01426dd0 as reading *(this+0x0c) and testing mask 0x230 with a <10 type guard.",
                "Play 9.8.3 x86 Ghidra export decompiles VagOperationDelegate::getVagSettingAvailabilityForEcu at 0x016b4180 calling the setting vtable slot +0x3c before reading the AvailBy byte at setting +0x5c.",
                "Direct x86 assembly at 0x016b41c9 calls PLT 0x019836c0, which resolves to _ZN20VagOperationDelegate37getVagSettingAvailabilityForSubmoduleERKNSt6__ndk110shared_ptrI10VagSettingEERKNS1_I10VagEcuInfoEE.",
                "The exported x86 symbol body at 0x016c97d0 rechecks setting vtable slot +0x3c and throws IllegalArgumentException with text 'Non-submodule setting passed to getVagSettingAvailabilityForSubmodule' if false.",
                "The 0x016c97d0 body reads *(VagEcuInfo+0x10) as a begin/end vector of shared_ptr<VagEcuInfo>, walks 8-byte entries, skips null submodule objects and null *(submodule VagEcuInfo+0x08 full_scan_part_number), then calls the setting whitelist object at VagSetting+0x54 vtable slot +0x10.",
                "VagEcuInfo constructor 0x013c95b0 stores the submodule vector shared_ptr argument into object offsets +0x10/+0x14, matching the 0x016c97d0 availability read from VagEcuInfo+0x10.",
                "VagEcuInfo::getInfo at Play 9.8.3 x86 0x013c9ef0 labels the +0x08 string as full_scan_part_number; the same offset is used by the tag and submodule availability whitelist paths.",
                "The older ARM Ghidra adaptation export at 0x012786c8 has the same shape: iterate *(VagEcuInfo+0x10), use *(submodule VagEcuInfo+0x08), and call the setting whitelist at +0x54.",
            ),
        ),
        VagOperationDelegateAvailabilityRoute(
            avail_by_values=(2,),
            native_route="fallback/full_scan_part_number whitelist route",
            predicate_input="full_scan_part_number shared string from VagEcuInfo +0x08",
            ...
            evidence=(
                ...
                "AvailBy 2, and the invalid/default route for values outside the recovered switch, calls the setting StringWhitelist itemMatches with the full_scan_part_number string at VagEcuInfo +0x08.",
                "Play 9.8.3 x86 VagEcuInfo::getInfo at 0x013c9ef0 labels +0x08 as full_scan_part_number, +0x34 as full_scan_component_name, +0x3c as full_scan_sw_version, and +0x44 as full_scan_asam_dataset.",
